...
import { bubbleSortSteps } from "../algos/bubbleSortSteps";
...
export default function BubbleSortViz({ showNavbar = true, showNavigator = true }) {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const [input, setInput] = useState("28, 31, 35, 37, 40, 47, 48, 52");
  const [array, setArray] = useState([]);
  const svgRef = useRef(null);
  const { hasSavedState, savedState, saving, saveState, clearSavedState } = useVisualizationState('bubble-sort');

  const actions = useMemo(() => (array.length ? bubbleSortSteps(array) : []), [array]);
  ...
  useEffect(() => {
    ...
    let arr = [...array];
    const highlights = { compare: [], swap: [], sorted: [] };
    let message = "";
    let isDone = false;
  ...
  return (
    <div className={showNavbar 
      ...
      <div className={showNavbar ? "max-w-6xl mx-auto px-6 py-8" : "w-full"}>
        {showNavigator && <AlgorithmNavigator currentSlug="bubble-sort" />}
        {showNavbar && (
