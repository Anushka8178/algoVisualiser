...
import { binarySearchSteps } from "../algos/binarySearchSteps";
...
export default function BinarySearchViz({ showNavbar = true, showNavigator = true }) {
  ...
  const { hasSavedState, savedState, saving, saveState } = useVisualizationState('binary-search');

  const actions = useMemo(
    () => (array.length ? binarySearchSteps(array, target) : []),
    [array, target]
  );
  ...
  const {
    ...
    setSpeed,
    setIndex,
  } = usePlayer(actions, savedState?.speed || 400);

  useEffect(() => {
    if (savedState && !array.length) {
      setInput(savedState.input || input);
      if (savedState.target !== undefined) setTarget(savedState.target);
  ...
  const handleSaveProgress = () => {
    saveState({ input, target, array, index, speed });
  };
  ...
  useEffect(() => {
    ...
    let arr = [...array];
    let message = "";
    let highlights = { low: null, mid: null, high: null, found: null };

    for (let i = 0; i <= index && i < actions.length; i++) {
      const a = actions[i];

      if (a.type === "check") {
        highlights.low = a.low;
        highlights.mid = a.mid;
        highlights.high = a.high;
        message = `Checking middle element: ${arr[a.mid]}`;
      }

      if (a.type === "moveRight") {
        message = `Moving right → target > ${arr[a.mid]}`;
      }

      if (a.type === "moveLeft") {
        message = `Moving left → target < ${arr[a.mid]}`;
      }
    ...
    const messageStroke = highlights.found !== null 
      ? (isDark ? "rgba(0, 255, 136, 0.6)" : "rgba(5, 150, 105, 0.8)")
      : (isDark ? "rgba(34, 211, 238, 0.6)" : "rgba(6, 182, 212, 0.8)");
    const messageText = highlights.found !== null 
      ? (isDark ? "#00ff88" : "#059669")
      : (isDark ? "#67e8f9" : "#0891b2");
    const messageFilter = highlights.found !== null
      ? (isDark ? "drop-shadow(0 2px 8px rgba(0, 255, 136, 0.3))" : "drop-shadow(0 2px 8px rgba(5, 150, 105, 0.2))")
      : (isDark ? "drop-shadow(0 2px 8px rgba(34, 211, 238, 0.3))" : "drop-shadow(0 2px 8px rgba(6, 182, 212, 0.2))");
  ...
  const generateRandomArray = () => {
    ...
    const sortedUnique = [...new Set(randomArr)].sort((a, b) => a - b);
    setInput(sortedUnique.join(", "));
    setArray(sortedUnique);
    reset();
  };
  ...
  return (
    <div className={showNavbar 
      ...
      <div className={showNavbar ? "max-w-6xl mx-auto px-6 py-8" : "w-full"}>
        {showNavigator && <AlgorithmNavigator currentSlug="binary-search" />}
        {showNavbar && (
      ...
      <div className="mb-6 w-full max-w-2xl mx-auto text-center">
        <input
          ...
          placeholder="Enter sorted numbers (e.g., 10, 20, 30, 40)"
        />
        <input
          type="text"
          value={target}
            ...
            <div className={`backdrop-blur-md rounded-xl p-5 border shadow-xl ${
              ...
              <div className={`pt-4 border-t ${
                ...
                <div className={`flex flex-wrap justify-center gap-4 text-sm ${
                  ...
                  <span><span className="text-blue-400">🔵</span> Range</span>
