...
import { quickSortSteps } from "../algos/quickSortSteps";
...
export default function QuickSortViz({ showNavbar = true, showNavigator = true }) {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const [input, setInput] = useState("45, 23, 78, 12, 56, 89, 34");
  const [array, setArray] = useState([]);
  const svgRef = useRef(null);
  const { hasSavedState, savedState, saving, saveState } = useVisualizationState('quick-sort');

  const actions = useMemo(
    () => (array.length ? quickSortSteps(array) : []),
    [array]
  );
  ...
  useEffect(() => {
    ...
    let arr = [...array];
    const highlights = { compare: [], swap: [], sorted: [], pivot: null };
    let message = "";
    ...
    for (let i = 0; i <= index && i < actions.length; i++) {
      ...
      if (a.type === "pivot") {
        highlights.pivot = a.index;
        message = `Choosing ${arr[a.index]} as pivot`;
      }
      if (a.type === "markSorted") {
        highlights.sorted = (highlights.sorted || []).concat(a.index);
  ...
  return (
    <div className={showNavbar 
      ...
      <div className={showNavbar ? "max-w-6xl mx-auto px-6 py-8" : "w-full"}>
        {showNavigator && <AlgorithmNavigator currentSlug="quick-sort" />}
        {showNavbar && (
      ...
      <div className="mb-6 w-full max-w-2xl mx-auto text-center">
        <input
          ...
          placeholder="Enter numbers separated by commas (e.g., 45, 12, 78, 34)"
        />
        <button
            ...
            <div className={`backdrop-blur-md rounded-xl p-5 border shadow-xl ${
              ...
              <div className={`pt-4 border-t ${
                ...
                <div className={`flex flex-wrap justify-center gap-4 text-sm ${
                  ...
                  <span><span className="text-blue-400">🔵</span> Pivot</span>
