...
export default function InsertionSortViz({ showNavbar = true, showNavigator = true }) {
  ...
  const { hasSavedState, savedState, saving, saveState } = useVisualizationState('insertion-sort');
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr("fill", (_, i) => {
        ...
        if (highlights.insert === i) return "#8b5cf6";
        if (highlights.shift?.to === i || highlights.shift?.from === i) return "#ef4444";
        if (highlights.compare?.includes(i)) return "#f59e0b";
        if (highlights.select === i) return "#3b82f6";
      ...
      {array.length > 0 && (
        <>
          ...
          <svg
            ...
            width="800"
            height="350"
            className={`mt-4 rounded-xl shadow-xl backdrop-blur-md border ${
              isDark
